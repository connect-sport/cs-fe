...
export const articleSchema = z
  ...
  .refine((data) => {
    return (
      !data.fromDateTime ||
      !data.toDateTime ||
      data.fromDateTime.isBefore(data.toDateTime)
    );
  });

export const filteringAricleSchema = z
  .object({
    keyword: z.string().optional(),
    address: z.string().optional(),
    levels: z.array(z.array(z.string())).optional(),
    fromDateTime: z.custom<Dayjs>().optional(),
    toDateTime: z.custom<Dayjs>().optional(),
  })
  .refine((data) => {
    if (data.fromDateTime && data.toDateTime) {
      return data.fromDateTime.isBefore(data.toDateTime);
    }
